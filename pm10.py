...
def forecast_pm10_with_monthly_averages(df, lgbm_pm10, feature_scaler, n_days=60):
    """
    ...
    ae_recon_loss_pm10 = df['ae_recon_loss_pm10'].values if 'ae_recon_loss_pm10' in df.columns else np.zeros(len(df))
    ...
    for i in range(n_days):
        ...
        X = np.hstack([
            ae_recon_loss_pm10[-1:].reshape(-1,1),  # Use last known value
            new_row[['NO2', 'Wind Speed', 'Humidity', 'month']].values,
            new_row[['Bhogi', 'Diwali', 'is_monsoon']].values
        ])
        
        # Predict PM10
        pm10_pred = lgbm_pm10.predict(X)[0]
        pm10_predictions.append(pm10_pred)
        
        # Update for next prediction
        if i < n_days - 1:
            forecast_df.at[i+1, 'PM10'] = pm10_pred
    
    # Add predictions to dataframe
    forecast_df['PM10_predicted'] = pm10_predictions
    return forecast_df
...
def train_pm10_model(df, ae_latent, ae_recon_loss):
    ...
    lgbm_pm10.fit(
        X_pm10[train_idx], 
        y_pm10[train_idx],
        eval_set=[(X_pm10[test_idx], y_pm10[test_idx])],
        eval_metric='rmse',
        callbacks=[lgb.early_stopping(stopping_rounds=5, min_delta=0.001)]
    )

    y_pred = lgbm_pm10.predict(X_pm10[test_idx])
    rmse = np.sqrt(mean_squared_error(y_pm10[test_idx], y_pred))
    mae = mean_absolute_error(y_pm10[test_idx], y_pred)
    
    print(f"PM10 Model - RMSE: {rmse:.2f}, MAE: {mae:.2f}")
    return lgbm_pm10, X_pm10
    
try:
    
    lgbm_pm10, X_pm10 = train_pm10_model(df, ae_latent, ae_recon_loss)
    #  Generate future predictions
    features = ['PM2.5','PM10', 'NO2', 'Wind Speed', 'Humidity']
    last_known = df[df['year'] == 2023].tail(30)
    pm10_forecast = forecast_pm10_with_monthly_averages(
    df=df,
    lgbm_pm10=lgbm_pm10,
    ...
    X_pm10_filtered = X_pm10[df_filtered.index]  # Use your PM10 feature matrix
    y_pm10_filtered = df_filtered['PM10'].values  # PM10 actual values
    predicted_pm10_filtered = lgbm_pm10.predict(X_pm10_filtered)  # Using your PM10 model
    
    plt.figure(figsize=(16, 8))

# Historical data
    plt.plot(df_filtered['Timestamp'], y_pm10_filtered, 
            label='Actual PM10', color='blue')
    plt.plot(df_filtered['Timestamp'], predicted_pm10_filtered, 
            label='Predicted PM10', color='orange')
    plt.title('PM10 Forecasting with Chennai-Specific Features')
    plt.xlabel('Date')  
    plt.show
...
# PM10 forecast
    plt.title("Chennai Air Quality Monitoring & Forecasting For PM10 (2022-2024)")
    plt.legend()
    ...
    rmse = np.sqrt(mean_squared_error(y_pm10_filtered, predicted_pm10_filtered))
    mae = mean_absolute_error(y_pm10_filtered,predicted_pm10_filtered)
    r2 = r2_score(y_pm10_filtered, predicted_pm10_filtered)

    # Create a DataFrame for export
    pm10_hybrid_results = pd.DataFrame({
    ...
    print("\nHybrid PM10 Evaluation Metrics:")
    print(pm10_hybrid_results.round(4))

    # Save as CSV (to merge later)
    pm10_hybrid_results.to_csv("hybrid_pm10_metrics.csv", index=False)
